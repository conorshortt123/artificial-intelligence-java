        ...
        for(int row = 0; row < cellspan; row++) {
        	for (int col = 0; col < cellspan; col++){
        		...
        		if (imageIndex < 0){
        			g.setFill(BACKGROUND_COLOUR);
        			g.fillRect(x1, y1, size, size);
        			g.drawImage(sprites[7].getNext(), x1, y1); // Floor cell
        		}else if (imageIndex == 0){
//        			g.setFill(HEDGE_COLOUR);//Hedge is 0
//        			g.fillRect(x1, y1, size, size);
        			g.drawImage(sprites[6].getNext(), x1, y1); // Hedge cell
        		}
    			else if (imageIndex == 1 && !player.isAlive()){
    				g.drawImage(sprites[8].getNext(), x1, y1); // Draws dead player
        		}
        		else{
        			//g.drawImage(sprites[7].getNext(), x1, y1);
        			g.drawImage(sprites[imageIndex - 1].getNext(), x1, y1); // Draws all other sprites
        		}
        	}
        }
	}
