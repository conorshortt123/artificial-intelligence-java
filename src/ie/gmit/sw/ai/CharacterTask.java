...
public class CharacterTask extends Task<Void> {
	 ...
	 */
	public CharacterTask(GameModel model, char enemyID, int row, int col, Minotaur minotaur) {
		this.model = model;
	...
	public Void call() throws Exception {
		 ...
		 * unnecessary...).
		 */
		while (alive && player.isAlive()) {
			Thread.sleep(SLEEP_TIME);

			synchronized (model) {
				
				MinotaurState state = minotaur.getState(); // Get's the minotaur's state on each update
				
				if(state == MinotaurState.DEAD)
					alive = false; // Kills game character if state is dead
				...
				// Get distance between player and current enemy using formula c = sqrt((x1 - x2)^2 + (y1 - y2)^2)
				double dist = GetDistance(player.getRow(), player.getCol(), row, col);

				if (dist <= 1) {
					minotaur.setPlayerRange(1); // Sets the player range in the minotaur class to 1, this is passed into the nn and fcl as input.
					if (player.isAlive())
						EngageMinotaur(); // Player attacks minotaur if alive.
				} else
					minotaur.setPlayerRange(0); // Otherwise player is out of range.
				
				// Only executes valid move if minotaur isn't in attack state.
				if (model.isValidMove(row, col, temp_row, temp_col, enemyID)
					 ...
					 * character.
					 */
					minotaur.execute(); // all minotaur classes are an implementation of command.
				}
			}
		}
		// Minotaur is dead, award health
		System.out.println(minotaur.getClass().getSimpleName() + " was killed! Player awarded +20 health");
		player.setHealth(player.getHealth() + 20);

		return null;
	}

	// Causes player to attack minotaur
	private void EngageMinotaur() {
		player.attack(minotaur);
	}

	// Get's distance between two points
